        ...
        <div className="container relative z-10 text-center">
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-600">
            ...
            <p className="text-lg lg:text-xl text-(--text-secondary) max-w-2xl mx-auto leading-relaxed">
              Thoughts on software engineering, design patterns, and emerging
              technologies.
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-8">
            {articles.map((article, index) => (
              <div
                ...
                style={
                  { animationDelay: `${index * 0.1}s`, fillMode: "both" } as any
                }
              >
                <Link
                  href={`/articles/${article.slug}`}
                  className="card group block p-8 rounded-3xl bg-(--bg-secondary)/30 hover:bg-(--bg-secondary)/80 transition-all duration-300 relative overflow-hidden shadow-sm hover:shadow-xl hover:shadow-brand-blue/5 border border-transparent hover:border-brand-blue/10"
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex flex-col md:flex-row md:items-baseline justify-between gap-4 mb-4">
                      <h2 className="text-2xl font-bold text-(--text-primary) group-hover:text-brand-blue transition-colors w-full tracking-tight">
                        {article.title}
                      </h2>
                    </div>
                    <p className="text-base text-(--text-secondary) mb-6 line-clamp-2 leading-relaxed max-w-2xl">
                      {article.description}
                    </p>

                    <div className="flex items-center text-sm font-bold text-(--text-primary) group-hover:text-brand-blue transition-colors">
                      Read Article
                      <svg
