...
import { fetchGitHubStats, GitHubStats as GitHubStatsType, getCachedData, setCachedData } from '../lib/statsApi';
...
const LoadingSkeleton = () => (
  <div className="p-6 rounded-2xl bg-[var(--bg-secondary)]/30 border border-[var(--border-color)]">
    ...
    <div className="grid grid-cols-2 gap-3 mb-6">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="p-4 bg-[var(--bg-card)] rounded-xl animate-pulse h-20" />
      ))}
    </div>
...
const StatCard = ({ 
  ...
  <motion.div
    ...
    className="relative p-4 rounded-xl bg-[var(--bg-card)]/50 border border-[var(--border-color)] hover:border-purple-500/30 transition-all group"
  >
    <div className="flex items-center gap-2 mb-2">
      <div className={`text-lg ${gradient}`}>
        {icon}
      </div>
...
const RepoCard = ({ repo, index }: { repo: any; index: number }) => {
  ...
  return (
    <motion.a
      ...
      className="block p-4 rounded-xl bg-[var(--bg-card)]/30 border border-[var(--border-color)] hover:border-purple-500/40 hover:bg-[var(--bg-card)]/50 transition-all group"
    >
      <div className="flex items-start justify-between gap-3 mb-2">
        <h4 className="font-bold text-sm text-[var(--text-primary)] group-hover:text-purple-400 transition-colors line-clamp-1">
          {repo.name}
        </h4>
        <svg className="w-3 h-3 text-[var(--text-secondary)] group-hover:text-purple-400 transition-colors flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function GitHubStats({ username, token }: GitHubStatsProps) {
  ...
  return (
    ...
    >
      {/* Card */}
      <div className="relative p-6 rounded-2xl bg-gradient-to-br from-[var(--bg-secondary)]/50 to-[var(--bg-secondary)]/30 border border-[var(--border-color)] hover:border-purple-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/5">
        {/* Header */}
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500/20 to-purple-600/10 flex items-center justify-center group-hover:scale-110 transition-transform">
            <FaGithub className="text-purple-400" size={20} />
          </div>
          <div>
            <h3 className="font-bold text-lg text-[var(--text-primary)]">GitHub</h3>
            <a
        ...
        {/* Decorative gradient */}
        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 via-purple-400 to-purple-600 opacity-50 rounded-t-2xl" />
      </div>
    </motion.div>
  );
}
